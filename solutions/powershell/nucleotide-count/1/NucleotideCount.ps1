Function Get-NucleotideCount() {
    ...
    foreach ($letter in ($Strand -split '' | Where-Object { $_ -ne '' })) {
        if ($LetterCountTable.ContainsKey($letter)) {
            $LetterCountTable[$letter]++
        } else {
            Throw "Invalid nucleotide in strand: $letter"
        }
    }
    
    return $LetterCountTable
}
